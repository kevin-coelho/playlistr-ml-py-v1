...
# SPLIT DATA
x_train, x_test, y_train, y_test = train_test_split(data_arr, labels, test_size=0.25, random_state=0)
sc_X = StandardScaler()
...
clf = Perceptron(fit_intercept=False, tol=None, shuffle=False).fit(x_train_poly, y_train)
x_test_poly = PolynomialFeatures(degree=2).fit_transform(x_test)
clf_results = clf.predict(x_test_poly)
...
poly_classifier = SVC(kernel='poly', degree=2, gamma='scale')
poly_classifier.fit(x_train, y_train)
poly_results = poly_classifier.predict(x_train)
poly_accuracy = 1 - (np.sum([1 for idx, result in enumerate(clf_results) if result == y_test[idx]]) / y_test.shape[0])
