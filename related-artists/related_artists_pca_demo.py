...
CORPUS_FILE = os.path.join(RESULT_DIR, 'related_artist_glove_corpus.model')
GLOVE_MODEL_FILE = os.path.join(RESULT_DIR, 'related_artist_glove.model')
...
similar_colors = ['Lianne La Havas' for elem in similar]

# get a 2nd item randomly
#second_name, second_idx = random.choice(list(dictionary.items()))
second_name, second_idx = ('Tim McGraw', dictionary['Tim McGraw'])
while second_name == args.QUERY:
    second_name, second_idx = random.choice(list(dictionary.items()))
second = matrix[second_idx]
second_similar = glove.most_similar(second_name, number=30)[11:30:6]
...
second_similar_colors = ['Tim McGraw' for elem in similar]

# get a 3rd item randomly
# third_name, third_idx = random.choice(list(dictionary.items()))
third_name, third_idx = ('Bob Marley & The Wailers', dictionary['Bob Marley & The Wailers'])
while third_name == args.QUERY:
    third_name, third_idx = random.choice(list(dictionary.items()))
third = matrix[third_idx]
third_similar = glove.most_similar(third_name, number=9)[0:9:3]
...
third_similar_colors = ['Bob Marley' for elem in similar]
...
colors = np.array(['Lianne La Havas', 'Tim McGraw', 'Bob Marley'])
...
plt.title('Related artists: {}, Tim McGraw, Bob Marley & The Wailers'.format(args.QUERY))
plt.xlabel('X')
plt.ylabel('Y')
texts = []
